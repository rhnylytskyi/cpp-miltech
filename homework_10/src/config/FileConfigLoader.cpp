...
void FileConfigLoader::load(const std::filesystem::path& configPath, const std::filesystem::path& ammoSource)
{
  ...
  config.physicsTimeStep =
    j["simulation"].contains("physicsTimeStep") ? j["simulation"]["physicsTimeStep"].get<float>() : config.simTimeStep;
  config.timeScale = j["simulation"].contains("timeScale") ? j["simulation"]["timeScale"].get<float>() : 1.0f;
