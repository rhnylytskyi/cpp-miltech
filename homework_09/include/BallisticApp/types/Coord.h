...
struct Coord {
   ...
   *
   * @example
   * BallisticApp::Coord facingDir = {1.0f, 0.0f};
   * BallisticApp::Coord targetDir = (targetPos - dronePos).normalize();
   * if (facingDir.dot(targetDir) < 0.0f) {
   *     // Target is located behind the object
   * }
   */
