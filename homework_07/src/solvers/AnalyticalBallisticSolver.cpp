...
float AnalyticalBallisticSolver::calcHDistance(float t, float V0, const AmmoParams& ammo)
{
    ...
    float h = t * V0
        - (ammo.drag * std::pow(t, 2) * V0) / (2 * ammo.mass)
        + (std::pow(t, 3) * (6 * ammo.drag * g_gravity * ammo.lift * ammo.mass - 8 * std::pow(ammo.drag, 2) * (-1 + l2) * V0)) / (36 * std::pow(ammo.mass, 2))
        + (std::pow(t, 4) * (-6 * std::pow(ammo.drag, 2) * g_gravity * ammo.lift * (1 + l2 + l4) * ammo.mass
            + 3 * std::pow(ammo.drag, 3) * l2 * (1 + l2) * V0
            + 6 * std::pow(ammo.drag, 3) * l4 * (1 + l2) * V0))
        / (36 * (1 + l2) * (1 + l2) * std::pow(ammo.mass, 3))
        + (std::pow(t, 5) * (3 * std::pow(ammo.drag, 3) * g_gravity * std::pow(ammo.lift, 3) * ammo.mass
            - 3 * std::pow(ammo.drag, 4) * l2 * (1 + l2) * V0))
        / (36 * (1 + l2) * std::pow(ammo.mass, 4));
